...
def main():
    ...
    L,R,V = [list(i) for i in zip(*_L)]

    # Step #2. 初期化
    dp = [INF for _ in range(W+2)]
    Z = segtree([INF] * (W+2), max, INF)
    dp[0] = 0
    Z.set(0, 0)

    # Step #3. 動的計画法
    for i in range(1,N+1):
        dp2 = [INF for _ in range(W+2)]
        for j in range(W+1):
            dp2[j] = dp[j]
            cl = max(0, j - R[i-1])
            cr = max(0, j - L[i-1] + 1)
            ...
            val = Z.prod(cl, cr)
            if val != INF:
                dp2[j] = max(dp2[j], val + V[i-1])
        dp = dp2
        if i < N:
            Z2 = segtree([INF] * (W+2), max, INF)
            Z2.set(0, 0)
            for j in range(W+1):
                Z2.set(j, dp2[j])
            Z = Z2
